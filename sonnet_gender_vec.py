...
class DebiasLayer(nn.Module):
    ...
    def forward(self, embeddings):
        ...
        gender_direction = self.gender_subspace.view(-1, 1)
...
class SonnetGPT(nn.Module):
  ...
  def forward(self, input_ids, attention_mask, target_agency="neutral"):
    ...
    if self.debias_layer:
      hidden_states = self.debias_layer(hidden_states)
...
if __name__ == "__main__":
  args = get_args()
  args.filepath = f'{args.epochs}-{args.lr}-sonnet_gender_vec.pt'  # Save path.
